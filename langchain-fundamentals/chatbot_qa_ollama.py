...
def scrape_docs(urls: List[str]) -> List[Dict]:
    ...
    try:
        all_docs = []
        for url in urls:  # ← Загружаем по одному
            loader = WebBaseLoader(web_path=url)  # ← Изменено!
            docs = loader.load()
            print(f"Loaded: {url} - {len(docs[0].page_content)} characters")
            all_docs.extend(docs)  # ← Добавляем к общему списку
