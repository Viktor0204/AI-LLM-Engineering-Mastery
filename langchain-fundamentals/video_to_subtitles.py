...
class YoutubeVideoSummarizer:
    ...
    def process_local_audio(self, audio_path: str) -> Dict:
        """Process local MP3 file without downloading"""
        try:
            if not os.path.exists(audio_path):
                raise FileNotFoundError(f"File not found: {audio_path}")

            print(f"Processing local file: {audio_path}")
            video_title = os.path.basename(audio_path).replace('.mp3', '').replace('.m4a', '')
