...
class YoutubeVideoSummarizer:
    ...
    def _setup_qa_chain(self):
        ...
        if self.vector_store is not None:
            ...
            self.qa_chain = ConversationalRetrievalChain.from_llm(
                ...
                memory=memory,
                verbose=False,  # Убрал verbose для чистоты
            )
    ...
    def transcribe_with_timestamps(self, audio_path: str, video_title: str) -> tuple[str, str]:
        ...
        # 2. Переводим на русский через LLM с индикатором прогресса
        print("Translating subtitles to Russian...")
        translated_segments = self._translate_segments_with_progress(segments, total_segments)

        if self.subtitle_format == "1":
            self._save_srt(translated_path, translated_segments)
        else:
            self._save_vtt(translated_path, translated_segments)
        print(f"\n✅ Translated subtitles saved to: {translated_path}")

        return original_path, translated_path

    def _translate_segments_with_progress(self, segments, total_segments):
        """Переводит сегменты с отображением прогресса в процентах"""
        translated = []
        ...
        for i, segment in enumerate(segments):
            ...
            try:
                ...
                # Отображаем прогресс
                if (i + 1) % max(1, total_segments // 10) == 0 or i + 1 == total_segments:
        ...
        # Если последний процент не 100, показываем 100%
        if int((len(translated)) / total_segments * 100) != 100:
            print("100%")
