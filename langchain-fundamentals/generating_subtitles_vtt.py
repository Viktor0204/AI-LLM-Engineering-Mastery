...
class YoutubeVideoSummarizer:
    ...
    def transcribe_with_timestamps(self, audio_path: str, video_title: str) -> tuple[str, str]:
        ...
        # Пути для файлов субтитров
        original_vtt_path = f"subtitles/{safe_title}_{detected_lang}.vtt"
        translated_vtt_path = f"subtitles/{safe_title}_ru.vtt"

        # Получаем сегменты
        segments = self._split_into_segments(result["segments"])

        # 1. Создаем оригинальные субтитры
        self._save_vtt(original_vtt_path, segments)
        print(f"✅ Original subtitles saved to: {original_vtt_path}")
        ...
        self._save_vtt(translated_vtt_path, translated_segments)
        print(f"✅ Translated subtitles saved to: {translated_vtt_path}")

        return original_vtt_path, translated_vtt_path
    ...
    def _format_timestamp(self, seconds):
        """Преобразует секунды в формат VTT (HH:MM:SS.mmm) с точкой"""
        ...
        # Формат с точкой: HH:MM:SS.mmm (как в вашем примере)
